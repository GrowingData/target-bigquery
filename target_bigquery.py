...
def persist_lines_job(project_id, dataset_id, lines=None, truncate=False, validate_records=True):
    ...
    for line in lines:
        ...
        if isinstance(msg, singer.RecordMessage):
            ...
            tables[table] = bigquery.Table(bigquery_client.dataset(dataset_id).table(table), schema=build_schema(schemas[table]))
    ...
    for table in rows.keys():
        ...
        data = rows[table]
        logger.info(f"table_ref: {table_ref}, config: {load_config}, data: {data}")

        try:
            load_job = bigquery_client.load_table_from_file(data, table_ref, job_config=load_config)
...
def collect():
    try:
        ...
        conn.connect()
        params = {"e": "se", "aid": "singer", "se_ca": "target-bigquery", "se_ac": "open", "se_la": version}
